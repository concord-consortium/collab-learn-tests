describe('Test student join a group', function(){
    ...
    it('will verify student is an specified group', function(){
        cy.get('.header > .group > .name').should('contain','Group '+group1);
    ...
    it('will will verify that both students are listed in the group in the Join Group group list and will create a new group when a student selects a different group and verify workspace header is correct', function(){
        ...
        cy.get('[value="Create Group"]').click();
        cy.get('.header > .group > .name').should('contain','Group '+group2);
        cy.get('.header > .user > .name').should('contain','Student '+student3);
        cy.get('.header > .group > .members > .member').should('contain','S'+student3);
        cy.get('.header > .group > .members > .member').should('not.contain','S'+student2).and('not.contain','S'+student1);
    });
    it('will verify no additional students can join group',function(){
        setup(student4);
        cy.get('.groups > .group-list > .group').contains(group1).click();
        cy.get('.header > .group > .name').should('contain','Group '+group1);
        cy.get('.header > .user > .name').should('contain','Student '+student4);
        cy.get('.header > .group > .members > .member').should('contain','S'+student1).and('contain','S'+student2).and('contain','S'+student4);
        setup(student5);
        cy.get('.groups > .group-list > .group').contains(group1).click();
        cy.get('.header > .group > .name').should('contain','Group '+group1);
        cy.get('.header > .user > .name').should('contain','Student '+student5);
        cy.get('.header > .group > .members > .member').should('contain','S'+student1).and('contain','S'+student2).and('contain','S'+student4).and('contain','S'+student5);
        setup(student6);
        cy.get('.groups > .group-list > .group').contains(group1).click();
    ...
    it('will verify a student can switch groups',function(){
        ...
        cy.get('.header > .group > .name').should('contain','Group '+group2);
        cy.get('.header > .user > .name').should('contain','Student '+student5);
        cy.get('.header > .group > .members > .member').should('contain','S'+student3).and('contain','S'+student5);
    });
    it('will verify new student can join group when one leaves it', function(){
        ...
        cy.get('.groups > .group-list > .group').contains(group1).click();
        cy.get('.header > .group > .name').should('contain','Group '+group1);
        cy.get('.header > .user > .name').should('contain','Student '+student6);
        cy.get('.header > .group > .members > .member').should('contain','S'+student1).and('contain','S'+student2).and('contain','S'+student4).and('contain','S'+student6);
    });
});